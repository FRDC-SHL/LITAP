...
```{r}
flow_mapper(file = "testELEV.dem", nrow = 150, ncol = 150)
```
...
```{r}
flow_mapper(file = "testELEV.dem", nrow = 150, ncol = 150, folder_out = "./Output/")
```
...
```{r}
flow_mapper(file = "testELEV.dem", nrow = 150, ncol = 150, folder_out = "./Output/", clean = TRUE)
```
...
```{r}
flow_mapper(file = "testELEV.dem", nrow = 150, ncol = 150, verbose = FALSE)
```

To prevent all output, use `quiet = TRUE`:

```{r}
flow_mapper(file = "testELEV.dem", nrow = 150, ncol = 150, quiet = TRUE)
```
