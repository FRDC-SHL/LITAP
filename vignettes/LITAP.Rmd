...
```{r}
complete_run(file = "testELEV.dem", nrow = 150, ncol = 150)
```
...
```{r}
complete_run(file = "testELEV.dem", nrow = 150, ncol = 150, folder_out = "./Output/")
```
...
```{r}
complete_run(file = "testELEV.dem", nrow = 150, ncol = 150, folder_out = "./Output/", clean = TRUE)
```
...
```{r, eval = FALSE}
complete_run(file = "testELEV.grd") # Grid files
complete_run(file = "testELEV.csv") # CSV files
complete_run(file = "testELEV.asc") # Ascii Grid files
complete_run(file = "testELEV.flt") # Floating point raster files
complete_run(file = "testELEV")     # ArcGis Gridfile folder (contains .hdr files)
```
...
```{r}
complete_run(file = "testELEV.dem", nrow = 150, ncol = 150, verbose = FALSE)
```

To prevent all output, use `quiet = TRUE`:

```{r}
complete_run(file = "testELEV.dem", nrow = 150, ncol = 150, quiet = TRUE)
```
