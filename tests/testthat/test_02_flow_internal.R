...
test_that("Sub-functions", {
  ...
  expect_equal_to_reference(d, "./ref/test_calc_ddir.rds")

  # Watersheds"
  expect_silent(d <- calc_shed4(d))
  expect_equal_to_reference(d, "./ref/test_calc_shed.rds")

  #Initial Pit Removal
  expect_silent(d_local <- first_pitr1(d, max_area = 10, max_depth = 5))
  expect_equal_to_reference(d, "./ref/test_local.rds")

  # Pond Pit Removal
  expect_silent(d_pond <- second_pitr1(d_local))
  expect_equal_to_reference(d, "./ref/test_pond.rds")
  ...
  expect_equal_to_reference(d, "./ref/test_fill.rds")

  # Inversion
  expect_silent(d <- invert(f))
  expect_equal_to_reference(d, "./ref/test_iinitial.rds")

  # Inverted directions
  expect_message(d <- calc_ddir2(d), NA)
  expect_equal_to_reference(d, "./ref/test_iddir.rds")

  # Inverted Watersheds
  expect_silent(d <- calc_shed4(d))
  expect_equal_to_reference(d, "./ref/test_ished.rds")

  # Inverted Initial Pit Removal
  expect_silent(d <- first_pitr1(d, max_area = 10, max_depth = 5))
  expect_equal_to_reference(d, "./ref/test_ilocal.rds")
})
