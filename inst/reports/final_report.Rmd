...
dem <- readRDS(stringr::str_subset(files, "_dem_fill.rds"))
pit <- readRDS(stringr::str_subset(files, paste0(run, "_pit.rds")))
pond <- readRDS(stringr::str_subset(files, paste0(run, "_pond.rds")))
fill <- readRDS(stringr::str_subset(files, paste0(run, "_fill.rds")))
```
...
```{r flow_plots}
grid.arrange(flow_plot(db = dem, type = "elevation", shed = TRUE, shed_type = "initial") + 
...
```{r pit_table}
DT::datatable(pit, rownames = FALSE)
```

### Watershed stats - Pond Table
```{r pond_table}
DT::datatable(pond, rownames = FALSE)
```

### Watershed stats - Fill Table
```{r fill_table}
DT::datatable(fill, rownames = FALSE, filter = "none", options = list(ordering=F))
```
